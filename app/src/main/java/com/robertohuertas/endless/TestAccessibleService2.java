...
public class TestAccessibleService2 extends AccessibilityService {
    private static final String TAG = GlobalConstant.LOG_TAG;
    private static final String TASK_LIST_VIEW_CLASS_NAME =
            "com.robertohuertas.endless.TestAccessibleService2";
    FrameLayout mLayout;
    WindowManager wm;
    String parameter;
    int foo;
...
//        if(current_time.equals(Common.thoigian)){
...
//            Log.d("@@","??????????????????????????=1: "+findFirstViewByText(nodeInfo, "Bắt đầu ngay"));
            findFirstViewByText(nodeInfo, "Bắt đầu ngay");
//            Log.d("@@","??????????????????????????=3: "+findFirstViewByText(nodeInfo, "NGĂN"));
//            Log.d("@@","??????????????????????????"+findFirstViewByText(nodeInfo));
//            if(isActivity) {
//                if (nodeInfox != null) {
//                    Log.d("@@","CON CAC DU MA MAY CON CAC DU MA MAY CON CAC DU MA MAY");
//                    nodeInfox.performAction(AccessibilityNodeInfo.ACTION_CLICK);
//                }
//            }
        }
